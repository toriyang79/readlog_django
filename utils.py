...
def save_book_if_needed(title, author, cover_url, isbn=None):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT * FROM books WHERE title=? AND ifnull(author,'')=ifnull(?, '')", (title, author))
    found = cur.fetchone()
    if found:
        conn.close()
...
def list_posts(limit=50, offset=0):
    ...
    cur.execute(
        """
        SELECT p.*, u.nickname, u.profile_image, b.title AS book_title, b.author AS book_author
        FROM posts p
...
def toggle_like(user_id, post_id):
    ...
    if row:
        with conn:
            ...
            conn.execute("UPDATE posts SET like_count = like_count - 1 WHERE id=? AND like_count>0", (post_id,))
        conn.close()
        return False
    else:
        with conn:
            conn.execute("INSERT INTO likes(user_id, post_id, created_at) VALUES(?,?,?)", (user_id, post_id, now_str()))
...
def do_repost(user_id, post_id):
    ...
    if row:
        ...
        return False
    with conn:
        conn.execute("INSERT INTO reposts(user_id, post_id, created_at) VALUES(?,?,?)", (user_id, post_id, now_str()))
...
def add_comment(user_id, post_id, text):
    conn = get_conn()
    with conn:
        conn.execute("INSERT INTO comments(user_id, post_id, text, created_at) VALUES(?,?,?,?)", (user_id, post_id, text, now_str()))
    conn.close()
...
def my_posts(user_id):
    ...
    cur.execute(
        """
        SELECT p.*, b.title AS book_title, b.author AS book_author
        FROM posts p
        LEFT JOIN books b ON p.book_id = b.id
        WHERE p.user_id=?
        ORDER BY datetime(p.created_at) DESC
...
def unread_notifications_count(user_id):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT COUNT(*) AS cnt FROM notifications WHERE user_id=? AND is_read=0", (user_id,))
